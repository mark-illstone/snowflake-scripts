...
THEN 
    ...
    COALESCE(split_order_refund.local_order_refund, 0),2)
ELSE 
    ROUND(
    COALESCE(msf.price, line_items.local_price, 0) + 
    COALESCE(line_items.local_addon_price, 0)  + 
...
THEN 
    ...
    COALESCE((100 - orders.sales_tax_percentage) / 100, 1),2)
...
THEN  
    ...
    COALESCE(split_discount.local_split_addon_discount, 0),2)
WHEN LOWER(orders.promo_code) LIKE '%bulk%'
THEN 
    ROUND(((
    COALESCE(msf.price, line_items.local_price, 0) + 
