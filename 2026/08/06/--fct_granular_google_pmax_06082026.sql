--fct_granular_google_pmax_06082026

CREATE OR REPLACE TABLE bi.mark_dev.fct_granular_google_pmax AS

WITH source AS ( select * from bi.fivetran_granular_google_ads_3.granular_pmax_daily)
...
, renamed AS (
    ...
    ON lower(s.product_custom_attribute_2) = lower(aa.acronym)
    LEFT JOIN product_categories p
    ON LOWER(aa.final_acronym) = lower(p.acronym)
    GROUP BY ALL
)

SELECT * FROM renamed where campaign_advertising_channel_type = 'PERFORMANCE_MAX'