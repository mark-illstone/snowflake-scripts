create or replace table bi.mark_dev.int_facebook_categorised as
...
SELECT
    ...
    TRIM(COALESCE(NULLIF(pc_by_product.keepsake_kids, ''), NULLIF(pc_by_name.keepsake_kids, ''),
             CASE WHEN feed_product_id = 'Unattributed' THEN 'Not attributed' END, 'Acronym not matched'), ' ') AS keepsake_kids,
    TRIM(COALESCE(NULLIF(pc_by_product.subcategory, ''), NULLIF(pc_by_name.subcategory, ''),
             CASE WHEN feed_product_id = 'Unattributed' THEN 'Not attributed' END, 'Acronym not matched'), ' ') AS keepsake_kids_subcategory,

    NULL AS subcategory,

    TRIM(COALESCE(NULLIF(pc_by_product.id, ''), NULLIF(pc_by_name.id, ''),
             CASE WHEN feed_product_id = 'Unattributed' THEN 'Not attributed' END, 'Acronym not matched'), ' ') AS product_slug
