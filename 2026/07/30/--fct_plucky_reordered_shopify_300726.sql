--fct_plucky_reordered_shopify_300726

CREATE OR REPLACE TABLE bi.mark_dev.fct_plucky_reordered_shopify AS

WITH orders AS (SELECT * FROM bi.plucky_shopify.int_orders_deduplication)
    ,order_tag AS (SELECT * FROM bi.mark_dev.int_plucky_order_tag_shopify)
    ,line_items AS (SELECT * FROM bi.plucky_shopify.int_line_items_deduplcation)
    ,fulfillment_order_line AS (SELECT * FROM bi.plucky_shopify.int_fulfillment_order_line)
    ,fulfillment AS (SELECT * FROM bi.plucky_shopify.int_fulfillment)
    ,location AS (SELECT * FROM bi.plucky_shopify.int_location)
    ,shipments as (SELECT * FROM bi.plucky_shopify.int_shipments)
