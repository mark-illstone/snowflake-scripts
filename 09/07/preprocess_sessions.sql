/*
This model joins the cleaned models containing the sessions data as well as the channel data from preprocess_channels.
...
sessions AS (
    ...
    LEFT JOIN bi.DBT_PRODUCTION_GA4.preprocess_session_funnel_events funnel USING(session_id)
)
...
BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
 ...
 AS criteria_id,
    ...
    --    first_product_page_path_seen,
        ...
        coalesce(b.retail_category, c.retail_category, d.retail_category) as retail_category, 
        coalesce(b.retail_sub_category, c.retail_sub_category, d.retail_sub_category) as retail_sub_category,  
        coalesce(b.pdc, c.pdc, d.pdc) as pdc,
        coalesce(b.brand_launch_date, c.brand_launch_date, d.brand_launch_date) as brand_launch_date,
        coalesce(b.trading_category, c.trading_category, d.trading_category) as trading_category,

        coalesce(b.evergreen_occasion_sub_cat, c.evergreen_occasion_sub_cat, d.evergreen_occasion_sub_cat) as evergreen_occasion_sub_cat,
        coalesce(b.keepsake_kids_sub_cat, c.keepsake_kids_sub_cat, d.keepsake_kids_sub_cat) as keepsake_kids_sub_cat,
        coalesce(b.keepsake_kids, c.keepsake_kids, d.keepsake_kids) as keepsake_kids,
...
left join skus_mapping_dedup_fallback d on d.brand = LOWER(REPLACE(REPLACE(REPLACE(sessions.first_product_seen, ' -', ''), ',', ''), ' ', '-'))
