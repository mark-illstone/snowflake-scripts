--int_hn_unit_cogs_shopify_07082026
...
, temp_unit_cogs as (
 ...
 SELECT 
     ...
     , psp_fulfilment_cost + psp_twistwrap_cost + hn_twistwrap_cost + COALESCE(hn_insert_cost, 0) as local_order_cost
     , psp_unit_cost
     , hn_unit_cost
     , psp_fulfilment_cost
     , psp_twistwrap_cost
...
, temp_cogs_ratio AS --splits the order level cogs metrics across all product lines on an order
(
SELECT
      a.line_item_id
    , a.order_id
    , coalesce(replace(a.local_order_cost, ',','')::FLOAT, 0) * NULLIF(b.ratio_to_order, 0) as local_order_cost
    , coalesce(replace(a.psp_fulfilment_cost, ',','')::FLOAT, 0) * NULLIF(b.ratio_to_order, 0) as psp_fulfilment_cost
    , coalesce(replace(a.psp_twistwrap_cost, ',','')::FLOAT, 0)  * NULLIF(b.ratio_to_order, 0) as psp_twistwrap_cost
    , coalesce(replace(a.hn_twistwrap_cost, ',','')::FLOAT, 0)  * NULLIF(b.ratio_to_order, 0) as hn_twistwrap_cost
    , coalesce(replace(a.hn_insert_cost, ',','')::FLOAT, 0)  * NULLIF(b.ratio_to_order, 0) as hn_insert_cost
FROM
    temp_cogs_group a
...
SELECT 
    ...
    , c.local_order_cost    as local_order_cost
    , a.psp_unit_cost       as local_psp_unit_cost
    , a.hn_unit_cost        as local_hn_unit_cost
    , c.psp_fulfilment_cost as local_psp_fulfilment_cost
    , c.psp_twistwrap_cost  as local_psp_twistwrap_cost
    , c.hn_twistwrap_cost   as local_hn_twistwrap_cost
    , a.psp_box_cost        as local_psp_box_cost
    , a.hn_box_cost         as local_hn_box_cost
    , c.hn_insert_cost      as local_hn_insert_cost
    
    , a.local_unit_cost     / b.rate  as unit_cost
    , c.local_order_cost    / b.rate  as order_cost
    , a.psp_unit_cost       / b.rate  as psp_unit_cost
    , a.hn_unit_cost        / b.rate  as hn_unit_cost
    , c.psp_fulfilment_cost / b.rate  as psp_fulfilment_cost
    , c.psp_twistwrap_cost  / b.rate  as psp_twistwrap_cost
    , c.hn_twistwrap_cost   / b.rate  as hn_twistwrap_cost
    , a.psp_box_cost        / b.rate  as psp_box_cost
    , a.hn_box_cost         / b.rate  as hn_box_cost
    , c.hn_insert_cost      / b.rate  as hn_insert_cost
FROM temp_cogs_group a
    INNER JOIN fx b
        ON b.currency = a.local_currency
            AND b.date = a.created_at::DATE
    LEFT JOIN temp_cogs_ratio c
        ON a.line_item_id = c.line_item_id
GROUP BY ALL